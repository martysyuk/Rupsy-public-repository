...
class VKAuth(object):
    def __init__(self, permissions, app_id, api_v, email=None, pswd=None, two_factor_auth=False, security_code=None,
                 auto_access=True, show_error=False):
        """
        @args:
    ...
    def _response_error(self, _response):
    ...
    def get_response(self, _method, _params):
        _params.update({'access_token': self._access_token,
                        'v': self.api_v
                        })
        while True:
            ...
            try:
                ...
                if self._response_error(_response):
                    continue
                else:
                    return False
